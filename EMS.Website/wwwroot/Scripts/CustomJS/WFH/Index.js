    ...
    function manageWFH() {
        ...
        function initializeForm() {
            ...
            $('.searchfilter').on('change', function () {
                if ($('#selfWFH').length > 0 && $('#selfWFH').prop('checked')) {
                    $('#user').val('');
        ...
        function GetProjectManagerUsers() {
            ...
            $.ajax
                ({
                    url: domain + 'WFH/GetEmployeesByPM',
                    type: 'POST',
                    data: {pmid: pmid},
        ...
        function LoadWFHGrid() {
            ...
            endDate = $('#EndDate').val();
            var data = { user: (($('#selfWFH').length > 0 && $('#selfWFH').prop('checked')) ? $('#selfWFH').val() : $('#user').val()), status: $('#status').val(), WFHcatagory: $('#WFHcatagory').val(), pm: $('#pm').val(), startDate: $('#StartDate').val(), endDate: $('#EndDate').val() };
            grid = new Global.GridHelper('#grid-NormalWFH', {
                serverSide: true,
                destroy: true,
                "pageLength": 50,
                "bFilter": false,
                ...
                columns: [
                    ...
                    { name: 'wfhId', data: 'wfhId', title: "wfhId", sortable: false, searchable: false, visible: false },
                    ...
                    //{
                    //    name: 'WFHCategory', data: 'WFHCategory', title: "WFH Category", sortable: false, searchable: false, render: function (data, type, full, meta){
                    //        return '<div style="max-width:200px;">' + data + '</div>';
                    //    }
                    //},
                    {
                        name: 'Comment', data: 'comment', title: "Comment", sortable: false, searchable: false, render: function (data, type, full, meta) {
                            return '<div style="max-width:300px;">' + data + '</div>';
                        }
                    },
                    //{ name: 'IsHalf', data: 'isHalf', title: "Leave Type", sortable: false, searchable: false },
                    { name: 'Team Manager', data: 'pmName', title: "Team Manager ", sortable: true, searchable: false },   
                    { name: 'Status', data: 'status', title: "Status ", sortable: true, searchable: false },
                    { name: 'Approved By', data: 'approvedBy', title: "Approved By ", sortable: true, searchable: false },   
                    { name: 'DateAdded', data: 'dateAdded', title: "Apply Date", sortable: true, searchable: false },
                    {
                        name: 'action', data: null, title: "Action", className: "text-center", sortable: false, searchable: false, render: function (data, type, full, meta) {
                            if (full.isEdit)
                                return '<a class="trans-btn"  href="' + domain + 'WFH/ManageWFH/' + full.wfhId + '" "><i class="fa fa-edit"></i></a>';
                            else
                                return '';
                        }
                    }
                ...
                "fnInitComplete": function (oSettings, json) {
                    ...
                    //if (json.totalWFH != undefined) {
                    //    var htmlWFH = '<table class="table table-stats"><tbody><tr><td colspan="3">Name: <b>' + json.totalWFH + '</b></td></tr>';
                    //    htmlWFH = htmlWFH + '<tr><td>From: <b>' + json.startDate + '</b></td><td colspan="2">To: <b>' + json.endDate + '</b></td></tr>';
                    ...
                    var html = json.recordsTotal != 0 ? '<a style="float:right;margin-left: 15px;" class="btn btn-warning" href="' + domain + 'WFH/downloadWFHdataexcel"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export Excel</a>' : '';
        ...
        function getfilter() {
            ...
            var data = {
                user: (($('#selfWFH').length > 0 && $('#selfWFH').prop('checked')) ? $('#selfWFH').val() : $('#user').val()),
                status: $('#status').val(),
               // leavetype: $('#leavetype').val(),
                WFHcatagory: $('#WFHcatagory').val(),
                pm: $('#pm').val(),
                startDate: $('#StartDate').val(),
                endDate: $('#EndDate').val()
            };
