    ...
    item.prototype = {
        ...
        constructor: function (params) {
            ...
            this._dumbCss = function (on) {
                ...
                // $(params.scroller).find("ul").css("background-color", "red");
                //alert($(params.scroller).find("ul").scrollHeight);
                //alert("Size: " + $(params.scroller).find("li").size());

                var liCount = $(params.scroller).find("li").size();

                //this.$(this.scroller).css(scrollerCss);
                if (liCount > 3) {
                    this.$(this.scroller).css(scrollerCss);
                    $(params.scroller).parent().find('.baron__track').removeClass('hidden');
                    $(params.scroller).parent().parent().parent().find('.main__scroller').css('height', '80px');
                    $(params.scroller).parent().parent().parent().find('.baron').css('height', '65px');
                }
                else {
                   // $(params.scroller).parent().find('.baron__track').remove();
                }
